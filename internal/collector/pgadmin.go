...
func EnablePgAdminLogging(ctx context.Context, instrumentationSpec *v1beta1.InstrumentationSpec,
	configmap *corev1.ConfigMap,
) error {
	if !feature.Enabled(ctx, feature.OpenTelemetryLogs) {
		return nil
	}
	otelConfig := NewConfig(instrumentationSpec)
	...
	if instrumentationSpec != nil &&
		instrumentationSpec.Logs != nil &&
		instrumentationSpec.Logs.Exporters != nil {
		exporters = instrumentationSpec.Logs.Exporters
	} else {
