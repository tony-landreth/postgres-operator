...
func NewConfig(instrumentationSpec *v1beta1.InstrumentationSpec) *Config {
	config := &Config{
		Exporters: map[ComponentID]any{
	...
	if instrumentationSpec != nil && instrumentationSpec.Config != nil &&
		instrumentationSpec.Config.Exporters != nil {
		for k, v := range instrumentationSpec.Config.Exporters {
